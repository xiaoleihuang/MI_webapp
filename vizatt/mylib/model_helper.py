...
def get_rnn_func(keras_model, config_att=None, input_mode=1):
	...
	rnn_layer_output_list = [K.function([keras_model.get_layer(config_att['content_input_layer']).input, K.learning_phase()], [keras_model.get_layer(config_att['content_output_layer']).output])]
	
	if input_mode == 2:
		# for context
		rnn_layer_output_list.append(
			K.function([keras_model.get_layer(config_att['context_input_layer']).input,K.learning_phase()], [keras_model.get_layer(config_att['context_output_layer']).output]))
...
def cal_pipeline(input_content, keras_model, input_context=None, input_mode=1, rnn_layer_func_list=None, rnn_layer_func_config=None):
